...
\usepackage[main, final]{neurips_2026} % using final here because author names were not visible due to neurips_2026 package by default
...
\usepackage{xcolor}
\usepackage{graphicx}
\usepackage{subfig} % for multi-panel figures
\graphicspath{{figures/}}

\title{How Has AI Research Changed? A Comparative Corpus Analysis of ArXiv Abstracts from 2007 to 2021}

\author{%
  Nithya [SURNAME] \And
  Rohan Bhardwaj \And
  Clarence [SURNAME] \And
  Shu [SURNAME] \And
  Chen [SURNAME] \\[4pt]
  Department of Data Science,
  University of Bristol \\
  \texttt{\{[student IDs or emails]\}@bristol.ac.uk}
}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.90\linewidth]{Report/figures/Comparative_Corpus_Analysis_Pipeline.png}
...
Our corpus comes from the \textsc{ArXiv} dataset \cite{arxiv_dataset}, which includes 27{,}121 abstracts of studies on artificial intelligence, machine learning, and natural language processing. We divided these abstracts into three non-overlapping groups based on when they were published: 2007-2011, 2012-2016, and 2017-2021. And these groups had abstracts of 1{,}268, 5{,}542 and 20{,}311 simultaneously. This clearly shows how quickly research is growing in this field, with more and more studies being published every year.
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/PCA_trajectory.png}
%     \caption{PCA trajectory of research abstracts in TF-IDF space (left) and SBERT-SPECTER space (right). Points are individual abstracts, larger markers are period centroids and arrows show the temporal direction of drift.}
%     \label{fig:pca_methods}
% \end{figure}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/Axis2_comparison3.png}
%     \caption{Axis 2 comparison for 2017-2021: TextRank top keyphrases (left) versus logistic regression discriminative coefficients (right). Green bars indicate terms strongly predictive of the period; red bars indicate terms strongly predictive against it (i.e., terms that belong to earlier periods).}
%     \label{fig:pca_methods}
% \end{figure}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/SBERT_framing_heatmap.png}
%     \caption{SBERT framing similarity heatmap. Each cell is the cosine similarity between the period centroid embedding and a hand-crafted archetype sentence. Darker shading indicates higher similarity.}
%     \label{fig:pca_methods}
% \end{figure}
...
Table~\ref{tab:centroid_distances_and_silhouette_score} and Figure~\ref{fig:centroid_combined} report centroid distances between successive periods for all five representations.
% \begin{table}[ht]
% \centering
% \caption{Centroid distances between successive temporal periods. TF-IDF and SBERT use cosine distance; LDA uses Euclidean distance in topic-proportion space. Mean shift = average across both transitions.}
% \label{tab:centroid_distances}
% \resizebox{\linewidth}{!}{%
% \begin{tabular}{lccc}
% \toprule
% \textbf{Method} & \textbf{2007--11 $\to$ 2012--16} &
%                   \textbf{2012--16 $\to$ 2017--21} &
%                   \textbf{Mean shift} \\
% \midrule
% TF-IDF (bigram, $V{=}5000$) & 0.2595 & 0.2889 & 0.2742 \\
% SBERT MiniLM                 & 0.0461 & 0.1221 & 0.0841 \\
% SBERT MPNet                  & 0.0500 & 0.1388 & 0.0944 \\
% SBERT SPECTER                & 0.0055 & 0.0298 & 0.0177 \\
% LDA ($K{=}12$)               & 0.0270 & 0.2733 & 0.1502 \\
% \bottomrule
% \end{tabular}}
% \end{table}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/axis1_grouped_bar_chart.png}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/axis1_transition_year_range_comparison.png}
...
% \end{figure}
% Combining to save space.
\begin{figure}[ht]
    \centering
    \subfloat[Centroid distances per transition]{%
        \includegraphics[width=0.48\linewidth]{Report/figures/axis1_grouped_bar_chart.png}
        \label{fig:centroid_bar}}
    \hfill
    \subfloat[Difference between transitions]{%
        \includegraphics[width=0.48\linewidth]{Report/figures/axis1_transition_year_range_comparison.png}
        \label{fig:transition_diff}}
    \caption{Axis~1 centroid distance results. (a) Distances between consecutive period pairs for all five methods; blue = first transition, red = second. (b) Difference (second minus first) confirming the 2012--2016 to 2017--2021 transition is larger across every method.}
    \label{fig:centroid_combined}
\end{figure}
...
% \begin{table}[ht]
% \centering
% \caption{Silhouette scores measuring separation of the three temporal periods in each representation space. Higher is better.}
...
% \midrule
% TF-IDF           & 0.0015 & vocabulary \\
% SBERT MiniLM     & 0.0541 & semantic embedding \\
% SBERT MPNet      & 0.0559 & semantic embedding \\
% SBERT SPECTER    & 0.0172 & semantic embedding \\
% LDA ($K{=}12$)   & 0.0029 & topic distribution \\
% \bottomrule
% \end{tabular}
% \end{table}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\linewidth]{Report/figures/silhouette_score.png}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/axis1_lda_topic_distribution.png}
...
% \begin{figure}[ht]
%     \centering
%     \includegraphics[width=\linewidth]{Report/figures/LDA_topics_shift.png}
...
% \end{figure}
% Here, we are combining 2 figures to fit number of pages
\begin{figure}[ht]
    \centering
    \subfloat[Topic proportions per period]{%
        \includegraphics[width=0.48\linewidth]{Report/figures/axis1_lda_topic_distribution.png}
        \label{fig:lda_topics}}
    \hfill
    \subfloat[Net topic change 2007-2011 to 2017-2021]{%
        \includegraphics[width=0.48\linewidth]{Report/figures/LDA_topics_shift.png}
        \label{fig:lda_change}}
    \caption{LDA topic analysis. (a) Stacked bar chart of topic proportions per period; each colour is one of 12 topics labelled by its three most probable words. (b) Net change in topic proportion: green = rising, red = declining. T4 (search/optimisation/optimal) and 
    T10 (inference/logic/probabilistic) show the largest declines; T9 (language/translation/languages) and T8 (neural/network/networks) show the largest rises.}
    \label{fig:lda_combined}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \subfloat[2007--2011]{%
        \includegraphics[width=0.32\linewidth]{Report/figures/Axis2_comparison1.png}
        \label{fig:axis2_2007}}
    \hfill
    \subfloat[2012--2016]{%
        \includegraphics[width=0.32\linewidth]{Report/figures/Axis2_comparison2.png}
        \label{fig:axis2_2012}}
    \hfill
    \subfloat[2017--2021]{%
        \includegraphics[width=0.32\linewidth]{Report/figures/Axis2_comparison3.png}
...
The results we derived from Axis 1 partially confirmed our initial hypothesis about lexical sensitivity and semantic sensitivity. As predicted, compared to the relatively smooth trajectory seen with SBERT (0.1245), TF-IDF detected a more significant centroid distance between neighbouring periods (peaking at 0.2889 between 2012–2016 and 2017–2021), demonstrating TF-IDF's high sensitivity to the sudden introduction of new vocabulary. However, the profile scores for both TF-IDF (0.0015) and SBERT (0.24) were close to 0. THis suggests that, although the vocabulary changed between periods, individual abstracts showed high diversity during the same period and could not be clearly classified by time. This reflects the continues evolution of concepts rather than distinct ,unrelated periods.
